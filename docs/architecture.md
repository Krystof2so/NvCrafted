...
## 7. Exclusion de `markdown` et `markdown_inline` de `nvim-treesitter` (Neovim ≥ 0.12)

### Décision

Les _parsers_ `markdown` et `markdown_inline` sont exclus de la gestion par `nvim-treesitter` dans `plugins/coding/treesitter.lua` :
...
Et dans `core/autocmds.lua`, une autocommande neutralise le _highlighter_ natif après les `ftplugins` :
...
**Neovim** 0.12 embarque ses propres _parsers_ **Tree-sitter** natifs, dont `markdown` et `markdown_inline`, activés automatiquement via `runtime/ftplugin/markdown.lua`. Ce fichier s'exécute dans la séquence de chargement avant que **nvim-treesitter** ne soit initialisé.

Quand les deux systèmes tentent de s'attacher au même _buffer_ **markdown**, le _parser_ se retrouve dans un état incohérent : **nvim-treesitter** tente d'appeler `range()` sur un nœud déjà partiellement construit par le _highlighter natif_, ce qui provoque :

```txt
attempt to call method 'range' (a nil value)
stack traceback:
    vim/treesitter/languagetree.lua:215
    vim/treesitter/languagetree.lua:596
    vim/treesitter/highlighter.lua:580
```

L'autocommande `FileType` dans `core/autocommands.lua` s'exécute après tous les _ftplugins_, ce qui garantit que le _highlighter_ natif est stoppé au bon moment, juste avant que **nvim-treesitter** ne tente de s'attacher.
